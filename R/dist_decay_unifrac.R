...
##----
## import unifrac distance matrix
##----
unifrac <- read.csv('./data/weighted-normalized.txt',
                 row.names = 1,
                 header = TRUE,
                 sep = '\t') %>%
  as.matrix()
...
colnames(unifrac) <- gsub('X', '', colnames(unifrac))

## match samples in unifrac matrix to those retained in mapping file
unifrac <- unifrac[rownames(map), rownames(map)]
...
tri.unifrac <- unifrac
diag(tri.unifrac) <- NA
tri.unifrac[lower.tri(tri.unifrac)] <- NA
...
gg.dat <- data.frame('unifrac' = na.omit(c(tri.unifrac)), 'gc' = na.omit(c(tri.gc)))
...
for(f in 1:length(windows)){
  ...
  ## subset unifrac distance matrix by index above
  unifrac.sub <- unifrac
  unifrac.sub[drop.em] <- NA
  unifrac.sub <- unifrac.sub %>% as.dist()
  ...
  ## mantel
  mant <- mantel(unifrac.sub, gc.sub, permutations = 999, na.rm = TRUE, parallel = cl)
...
## save output
write.csv(mant.results, './data/mantel-results-unifrac.csv')
...
## add binary variable if p is significant
mant.results$p.sig <- rep(0)
mant.results$p.sig[mant.results$p.adj < 0.06] <- 1 %>% factor()
...
## save figure
pdf('./figs/corr-plot-unifrac.pdf', width = 7, height = 6)
gg.corr
...
dd.100 <- ggplot(gg.dat.sub, aes(x = gc, y = unifrac, fill = ..count..)) +
  theme_classic() +
  ...
  stat_smooth(data = gg.dat.sub, aes(x = gc, y = unifrac),
...
dd.4500 <- ggplot(gg.dat, aes(x = gc, y = unifrac, fill = ..count..)) +
  theme_classic() +
  ylab('') +
  xlab('') +
  scale_y_continuous(breaks = pretty_breaks(n = 6)) +
  ...
  # stat_bin2d(bins = 20, drop = TRUE) +
  stat_smooth(data = gg.dat, aes(x = gc, y = unifrac),
              size = 0.5, linetype = 2, colour = 'black',
              method = 'glm', method.args = list(family = 'quasibinomial'),
              level = 0, inherit.aes = FALSE) +
...
## save figure
pdf('./figs/inset-plot-unifrac.pdf', width = 7, height = 6)
